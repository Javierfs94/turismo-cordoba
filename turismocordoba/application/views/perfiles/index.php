  ...
  switch ($datos->perfil) {
    case 1: // turista
      foreach ($perfil as $row) {
        ...
        if ($row->puntos < 25) {
          echo '
            <div class="progress-bar bg-danger" role="progressbar" style="width: ' . $row->puntos . '%" aria-valuenow="' . $row->puntos . '" aria-valuemin="0" aria-valuemax="100"></div>
          </div>
        ';
        }
        if ($row->puntos >= 25 && $row->puntos < 75) {
          echo '
        ...
        if ($row->puntos >= 75) {
          echo '
            <div class="progress-bar bg-success" role="progressbar" style="width: ' . $row->puntos . '%" aria-valuenow="' . $row->puntos . '" aria-valuemin="0" aria-valuemax="100"></div>
          </div>
        ';
        }
      }
